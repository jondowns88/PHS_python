...
/* 7/23/2020 (JD)*/
--SUD now going FFS. Removing SUD auths from query.
...
WITH auths AS(
	...
	, start_date
	, expire_date
	, CASE WHEN program IN('400', '401') THEN program --Already correct
	...
	FROM kcrsn.au_master AS a
	LEFT JOIN kcrsn.g_person AS c ON a.kcid = c.kcid
	LEFT JOIN kcrsn.au_initial_plan AS d ON a.auth_no = d.auth_no
	CROSS APPLY kcrsn.[p_get_case_rate_elements](a.kcid, a.auth_no, @calc_date) as crv
	WHERE program IN('3A1', '3B1', '2X1', '400', '401') --MH programs
		...
		AND a.auth_no NOT IN(SELECT auth_no FROM kcrsn.au_stratum WHERE start_date BETWEEN @qstart AND @qEnd) --Not already stratified
		AND (
...
LEFT JOIN kcrsn.g_p1_client ON a.kcid = g_p1_client.kcid --Get Medicaid ID (if applicable)